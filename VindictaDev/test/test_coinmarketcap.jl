...
_test_cmc_1(fromenv=true) = begin
    cmc = CoinMarketCap
    if fromenv && Base.get(ENV, "VINDICTA_CMC_APIKEY", "") != ""
        cmc.setapikey!(true)
    else
        config_path = joinpath(dirname(dirname(dirname(pathof(Vindicta)))), "user", "secrets.toml")
...
test_coinmarketcap(fromenv=true) = begin
    @eval begin
        using .Vindicta: Vindicta
        using .Vindicta.Engine.LiveMode.Watchers.CoinMarketCap
    end
